...
가장 먼저 구현할 기능은 `SessionVo`와 request header meta 값을 함께 사용해서 API 접근 권한을 제어하는 것이다.

중요한 기준은 다음과 같다.

- `SessionVo`에는 사용자 정보와 권한만 저장한다.
- `uuid`, `client`는 `SessionVo`에 저장하지 않는다.
- `uuid`, `client`는 보호된 API 요청마다 header meta 값으로만 받는다.
...
첫 구현에서는 Oracle 연결 설정을 `application.yml`에 넣어두되, 보안 샘플 기능은 실제 DB 데이터에 의존하지 않는다. 처음 학습할 때 요청 흐름, 세션 처리, meta header 검증, 권한 검증에 집중하기 위해서다.
...
세션 기반 요청이든 Swagger fallback 요청이든 보호된 API라면 두 header가 반드시 있어야 한다.
...
2. 세션에 값이 없으면 개발/Swagger 테스트용 요청 header를 확인한다.

개발용 `SessionVo` 생성 header는 다음과 같다.
...
header 기반 fallback은 설정으로 켜고 끌 수 있어야 한다.
...
- `SessionVo`가 존재해야 한다.
- request header `X-UUID`가 비어 있으면 안 된다.
- request header `X-CLIENT`가 비어 있으면 안 된다.
- `SessionVo`에 최소 한 개 이상의 권한이 있어야 한다.
- 사용자의 권한이 요청 API에 접근 가능한 권한이어야 한다.

`SessionVo`가 없거나 필수 meta header가 잘못된 경우 `401 Unauthorized`를 반환한다.

`SessionVo`는 있지만 권한이 부족한 경우 `403 Forbidden`을 반환한다.

Swagger, 샘플 로그인 API, 샘플 로그아웃 API 같은 공개 경로는 `SessionVo`와 meta header 없이 접근할 수 있다.
...
3. request header에서 `X-UUID`, `X-CLIENT`를 검증한다.
4. `SessionVo`의 권한 목록을 검증한다.
5. 권한 목록을 `GrantedAuthority`로 변환한다.
6. `Authentication` 객체를 만들어 `SecurityContextHolder`에 저장한다.
...
이 API는 `userId`, `userName`, `roles`만 담긴 `SessionVo`를 `HttpSession`에 저장하고, 저장된 세션 정보를 응답으로 반환한다.
...
Swagger UI에서 전역 header를 넣을 수 있게 설정한다.
...
1. `/api/auth/login-sample`을 먼저 호출해서 서버 세션 쿠키를 사용한다. 이후 보호 API에는 `X-UUID`, `X-CLIENT` header를 보낸다.
2. `app.security.header-session-enabled=true`일 때 `X-USER-ID`, `X-USER-NAME`, `X-ROLES`로 임시 `SessionVo`를 만들고, `X-UUID`, `X-CLIENT`로 meta 값을 검증한다.
...
- `GET /api/common/me`: 정상 `SessionVo`와 meta header가 있는 모든 사용자
- `GET /api/user/profile`: `USER`, `MANAGER`, `ADMIN`
- `GET /api/manager/report`: `MANAGER`, `ADMIN`
- `GET /api/admin/dashboard`: `ADMIN`

각 API는 현재 세션 정보와 짧은 메시지를 반환한다.
...
- 보호된 API에서 `X-UUID` 또는 `X-CLIENT`가 없으면 401을 반환한다.
- header fallback으로 `USER` 권한과 meta header를 보내면 `/api/user/profile`에 접근 가능하다.
- header fallback으로 `USER` 권한만 보내면 `/api/admin/dashboard` 접근은 403이 된다.
- 샘플 로그인 API로 세션을 만든 뒤 meta header를 보내면 세션 기반으로 보호된 API에 접근할 수 있다.
